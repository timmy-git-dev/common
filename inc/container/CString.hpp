...
public:
        CString();
        CString(const s64 _capacity, const s64 _length, const char &_copiedChar);
        CString(const s64 _capacity, const s64 _length, const char *_cString);

        ~CString          ();
        CString           (const CString  &_copied) = delete;
        CString           (      CString &&_moved ) = delete;
        CString &operator=(const CString  &_copied) = delete;
        CString &operator=(      CString &&_moved ) = delete;

        const c08 &operator[](s64 _index) const;
              c08 &operator[](s64 _index);

        bool operator==(const CString &_other) const;
        bool operator!=(const CString &_other) const;
        bool operator< (const CString &_other) const;
        bool operator<=(const CString &_other) const;
        bool operator> (const CString &_other) const;
