...
namespace cmn::system::xnu
{
    ...
    int recvmsg(int s,struct msghdr *msg,int flags) {return syscall(RECVMSG,(long)s,(long)msg,(long)flags);}
    int sendmsg(int s,caddr_t msg,int flags) {return syscall(SENDMSG,(long)s,(long)msg,(long)flags);}
    int recvfrom(int s,void *buf,size_t len,int flags,struct sockaddr *from,int *fromlenaddr) {return syscall(RECVFROM,(long)s,(long)buf,(long)len,(long)flags,(long)from,(long)fromlenaddr);}
    ...
    int sigaction(int signum,struct __sigaction *nsa,struct sigaction *osa) {return syscall(SIGACTION,(long)signum,(long)nsa,(long)osa);}
    ...
    int sigpending(struct sigvec *osv) {return syscall(SIGPENDING,(long)osv);}
    int sigaltstack(struct sigaltstack *nss,struct sigaltstack *oss)  {return syscall(SIGALTSTACK,(long)nss,(long)oss);}
    ...
    int setitimer(u_int which,struct itimerval *itv,struct itimerval *oitv) {return syscall(SETITIMER,(long)which,(long)itv,(long)oitv);}
    int swapon(void) {return syscall(SWAPON);}
    int getitimer(u_int which,struct itimerval *itv) {return syscall(GETITIMER,(long)which,(long)itv);}
    ...
    int select(int nd,u_int32_t *in,u_int32_t *ou,u_int32_t *ex,struct timeval *tv) {return syscall(SELECT,(long)nd,(long)in,(long)ou,(long)ex,(long)tv);}
    ...
    int gettimeofday(struct timeval *tp,struct timezone *tzp,uint64_t *mach_absolute_time) {return syscall(GETTIMEOFDAY,(long)tp,(long)tzp,(long)mach_absolute_time);}
    int getrusage(int who,struct rusage *rusage) {return syscall(GETRUSAGE,(long)who,(long)rusage);}
    ...
    user_ssize_t readv(int fd,struct iovec *iovp,u_int iovcnt) {return syscall(READV,(long)fd,(long)iovp,(long)iovcnt);}
    user_ssize_t writev(int fd,struct iovec *iovp,u_int iovcnt) {return syscall(WRITEV,(long)fd,(long)iovp,(long)iovcnt);}
    int settimeofday(struct timeval *tv,struct timezone *tzp) {return syscall(SETTIMEOFDAY,(long)tv,(long)tzp);}
    ...
    int utimes(char *path,struct timeval *tptr) {return syscall(UTIMES,(long)path,(long)tptr);}
    int futimes(int fd,struct timeval *tptr) {return syscall(FUTIMES,(long)fd,(long)tptr);}
    int adjtime(struct timeval *delta,struct timeval *olddelta) {return syscall(ADJTIME,(long)delta,(long)olddelta);}
    int gethostuuid(unsigned char *uuid_buf,const struct timespec *timeoutp) {return syscall(GETHOSTUUID,(long)uuid_buf,(long)timeoutp);}
    ...
    int statfs(char *path,struct statfs *buf) {return syscall(STATFS,(long)path,(long)buf);}
    int fstatfs(int fd,struct statfs *buf) {return syscall(FSTATFS,(long)fd,(long)buf);}
    ...
    int sigreturn(struct ucontext *uctx,int infostyle,user_addr_t token) {return syscall(SIGRETURN,(long)uctx,(long)infostyle,(long)token);}
    ...
    int getrlimit(u_int which,struct rlimit *rlp) {return syscall(GETRLIMIT,(long)which,(long)rlp);}
    int setrlimit(u_int which,struct rlimit *rlp) {return syscall(SETRLIMIT,(long)which,(long)rlp);}
    ...
    int sysctl(int *name,u_int namelen,void *old,size_t *oldlenp,void *new,size_t newlen) {return syscall(SYSCTL,(long)name,(long)namelen,(long)old,(long)oldlenp,(long)new,(long)newlen);}
    ...
    int open_dprotected_np(user_addr_t path,int flags,int class,int dpflags,int mode) {return syscall(OPEN_DPROTECTED_NP,(long)path,(long)flags,(long)class,(long)dpflags,(long)mode);}
    ...
    int openat_dprotected_np(int fd,user_addr_t path,int flags,int class,int dpflags,int mode,int authfd) {return syscall(OPENAT_DPROTECTED_NP,(long)fd,(long)path,(long)flags,(long)class,(long)dpflags,(long)mode,(long)authfd);}
    int getattrlist(const char *path,struct attrlist *alist,void *attributeBuffer,size_t bufferSize,u_long options) {return syscall(GETATTRLIST,(long)path,(long)alist,(long)attributeBuffer,(long)bufferSize,(long)options);}
    int setattrlist(const char *path,struct attrlist *alist,void *attributeBuffer,size_t bufferSize,u_long options) {return syscall(SETATTRLIST,(long)path,(long)alist,(long)attributeBuffer,(long)bufferSize,(long)options);}
    int getdirentriesattr(int fd,struct attrlist *alist,void *buffer,size_t buffersize,u_long *count,u_long *basep,u_long *newstate,u_long options) {return syscall(GETDIRENTRIESATTR,(long)fd,(long)alist,(long)buffer,(long)buffersize,(long)count,(long)basep,(long)newstate,(long)options);}
    ...
    int searchfs(const char *path,struct fssearchblock *searchblock,uint32_t *nummatches,uint32_t scriptcode,uint32_t options,struct searchstate *state) {return syscall(SEARCHFS,(long)path,(long)searchblock,(long)nummatches,(long)scriptcode,(long)options,(long)state);}
    int delete(user_addr_t path) {return syscall(DELETE,(long)path);}
    int copyfile(char *from,char *to,int mode,int flags) {return syscall(COPYFILE,(long)from,(long)to,(long)mode,(long)flags);}
    int fgetattrlist(int fd,struct attrlist *alist,void *attributeBuffer,size_t bufferSize,u_long options) {return syscall(FGETATTRLIST,(long)fd,(long)alist,(long)attributeBuffer,(long)bufferSize,(long)options);}
    int fsetattrlist(int fd,struct attrlist *alist,void *attributeBuffer,size_t bufferSize,u_long options) {return syscall(FSETATTRLIST,(long)fd,(long)alist,(long)attributeBuffer,(long)bufferSize,(long)options);}
    int poll(struct pollfd *fds,u_int nfds,int timeout) {return syscall(POLL,(long)fds,(long)nfds,(long)timeout);}
    ...
    int posix_spawn(pid_t *pid,const char *path,const struct _posix_spawn_args_desc *adesc,char **argv,char **envp) {return syscall(POSIX_SPAWN,(long)pid,(long)path,(long)adesc,(long)argv,(long)envp);}
    int ffsctl(int fd,u_long cmd,caddr_t data,u_int options) {return syscall(FFSCTL,(long)fd,(long)cmd,(long)data,(long)options);}
    int fhopen(const struct fhandle *u_fhp,int flags) {return syscall(FHOPEN,(long)u_fhp,(long)flags);}
    ...
    int semctl(int semid,int semnum,int cmd,semun_t arg) {return syscall(SEMCTL,(long)semid,(long)semnum,(long)cmd,(long)arg);}
    int semget(key_t key,int nsems,int semflg) {return syscall(SEMGET,(long)key,(long)nsems,(long)semflg);}
    int semop(int semid,struct sembuf *sops,int nsops) {return syscall(SEMOP,(long)semid,(long)sops,(long)nsops);}
    int msgctl(int msqid,int cmd,struct msqid_ds *buf) {return syscall(MSGCTL,(long)msqid,(long)cmd,(long)buf);}
    ...
    int shmctl(int shmid,int cmd,struct shmid_ds *buf) {return syscall(SHMCTL,(long)shmid,(long)cmd,(long)buf);}
    ...
    int sys_sysctlbyname(const char *name,size_t namelen,void *old,size_t *oldlenp,void *new,size_t newlen) {return syscall(SYS_SYSCTLBYNAME,(long)name,(long)namelen,(long)old,(long)oldlenp,(long)new,(long)newlen);}
    ...
    int sendfile(int fd,int s,off_t offset,off_t *nbytes,struct sf_hdtr *hdtr,int flags) {return syscall(SENDFILE,(long)fd,(long)s,(long)offset,(long)nbytes,(long)hdtr,(long)flags);}
    ...
    int statfs64(char *path,struct statfs64 *buf) {return syscall(STATFS64,(long)path,(long)buf);}
    int fstatfs64(int fd,struct statfs64 *buf) {return syscall(FSTATFS64,(long)fd,(long)buf);}
    ...
    int getaudit_addr(struct auditinfo_addr *auditinfo_addr,int length) {return syscall(GETAUDIT_ADDR,(long)auditinfo_addr,(long)length);}
    int setaudit_addr(struct auditinfo_addr *auditinfo_addr,int length) {return syscall(SETAUDIT_ADDR,(long)auditinfo_addr,(long)length);}
    ...
    int kevent(int fd,const struct kevent *changelist,int nchanges,struct kevent *eventlist,int nevents,const struct timespec *timeout) {return syscall(KEVENT,(long)fd,(long)changelist,(long)nchanges,(long)eventlist,(long)nevents,(long)timeout);}
    ...
    int kevent64(int fd,const struct kevent64_s *changelist,int nchanges,struct kevent64_s *eventlist,int nevents,unsigned int flags,const struct timespec *timeout) {return syscall(KEVENT64,(long)fd,(long)changelist,(long)nchanges,(long)eventlist,(long)nevents,(long)flags,(long)timeout);}
    ...
    int kevent_qos(int fd,const struct kevent_qos_s *changelist,int nchanges,struct kevent_qos_s *eventlist,int nevents,void *data_out,size_t *data_available,unsigned int flags) {return syscall(KEVENT_QOS,(long)fd,(long)changelist,(long)nchanges,(long)eventlist,(long)nevents,(long)data_out,(long)data_available,(long)flags);}
    int kevent_id(uint64_t id,const struct kevent_qos_s *changelist,int nchanges,struct kevent_qos_s *eventlist,int nevents,void *data_out,size_t *data_available,unsigned int flags) {return syscall(KEVENT_ID,(long)id,(long)changelist,(long)nchanges,(long)eventlist,(long)nevents,(long)data_out,(long)data_available,(long)flags);}
    int __mac_execve(char *fname,char **argp,char **envp,struct mac *mac_p) {return syscall(__MAC_EXECVE,(long)fname,(long)argp,(long)envp,(long)mac_p);}
    int __mac_syscall(char *policy,int call,user_addr_t arg) {return syscall(__MAC_SYSCALL,(long)policy,(long)call,(long)arg);}
    int __mac_get_file(char *path_p,struct mac *mac_p) {return syscall(__MAC_GET_FILE,(long)path_p,(long)mac_p);}
    int __mac_set_file(char *path_p,struct mac *mac_p) {return syscall(__MAC_SET_FILE,(long)path_p,(long)mac_p);}
    int __mac_get_link(char *path_p,struct mac *mac_p) {return syscall(__MAC_GET_LINK,(long)path_p,(long)mac_p);}
    int __mac_set_link(char *path_p,struct mac *mac_p) {return syscall(__MAC_SET_LINK,(long)path_p,(long)mac_p);}
    int __mac_get_proc(struct mac *mac_p) {return syscall(__MAC_GET_PROC,(long)mac_p);}
    int __mac_set_proc(struct mac *mac_p) {return syscall(__MAC_SET_PROC,(long)mac_p);}
    int __mac_get_fd(int fd,struct mac *mac_p) {return syscall(__MAC_GET_FD,(long)fd,(long)mac_p);}
    int __mac_set_fd(int fd,struct mac *mac_p) {return syscall(__MAC_SET_FD,(long)fd,(long)mac_p);}
    int __mac_get_pid(pid_t pid,struct mac *mac_p) {return syscall(__MAC_GET_PID,(long)pid,(long)mac_p);}
    int pselect(int nd,u_int32_t *in,u_int32_t *ou,u_int32_t *ex,const struct timespec *ts,const struct sigset_t *mask) {return syscall(PSELECT,(long)nd,(long)in,(long)ou,(long)ex,(long)ts,(long)mask);}
    int pselect_nocancel(int nd,u_int32_t *in,u_int32_t *ou,u_int32_t *ex,const struct timespec *ts,const struct sigset_t *mask) {return syscall(PSELECT_NOCANCEL,(long)nd,(long)in,(long)ou,(long)ex,(long)ts,(long)mask);}
    ...
    int recvmsg_nocancel(int s,struct msghdr *msg,int flags) {return syscall(RECVMSG_NOCANCEL,(long)s,(long)msg,(long)flags);}
    int sendmsg_nocancel(int s,caddr_t msg,int flags) {return syscall(SENDMSG_NOCANCEL,(long)s,(long)msg,(long)flags);}
    int recvfrom_nocancel(int s,void *buf,size_t len,int flags,struct sockaddr *from,int *fromlenaddr) {return syscall(RECVFROM_NOCANCEL,(long)s,(long)buf,(long)len,(long)flags,(long)from,(long)fromlenaddr);}
    ...
    int select_nocancel(int nd,u_int32_t *in,u_int32_t *ou,u_int32_t *ex,struct timeval *tv) {return syscall(SELECT_NOCANCEL,(long)nd,(long)in,(long)ou,(long)ex,(long)tv);}
    ...
    user_ssize_t readv_nocancel(int fd,struct iovec *iovp,u_int iovcnt) {return syscall(READV_NOCANCEL,(long)fd,(long)iovp,(long)iovcnt);}
    user_ssize_t writev_nocancel(int fd,struct iovec *iovp,u_int iovcnt) {return syscall(WRITEV_NOCANCEL,(long)fd,(long)iovp,(long)iovcnt);}
    ...
    int poll_nocancel(struct pollfd *fds,u_int nfds,int timeout) {return syscall(POLL_NOCANCEL,(long)fds,(long)nfds,(long)timeout);}
    ...
    int __mac_mount(char *type,char *path,int flags,caddr_t data,struct mac *mac_p) {return syscall(__MAC_MOUNT,(long)type,(long)path,(long)flags,(long)data,(long)mac_p);}
    int __mac_get_mount(char *path,struct mac *mac_p) {return syscall(__MAC_GET_MOUNT,(long)path,(long)mac_p);}
    ...
    int connectx(int socket,const sa_endpoints_t *endpoints,sae_associd_t associd,unsigned int flags,const struct iovec *iov,unsigned int iovcnt,size_t *len,sae_connid_t *connid) {return syscall(CONNECTX,(long)socket,(long)endpoints,(long)associd,(long)flags,(long)iov,(long)iovcnt,(long)len,(long)connid);}
    ...
    int necp_match_policy(uint8_t *parameters,size_t parameters_size,struct necp_aggregate_result *returned_result) {return syscall(NECP_MATCH_POLICY,(long)parameters,(long)parameters_size,(long)returned_result);}
    int getattrlistbulk(int dirfd,struct attrlist *alist,void *attributeBuffer,size_t bufferSize,uint64_t options) {return syscall(GETATTRLISTBULK,(long)dirfd,(long)alist,(long)attributeBuffer,(long)bufferSize,(long)options);}
    ...
    int getattrlistat(int fd,const char *path,struct attrlist *alist,void *attributeBuffer,size_t bufferSize,u_long options) {return syscall(GETATTRLISTAT,(long)fd,(long)path,(long)alist,(long)attributeBuffer,(long)bufferSize,(long)options);}
    ...
    user_ssize_t recvmsg_x(int s,struct msghdr_x *msgp,u_int cnt,int flags) {return syscall(RECVMSG_X,(long)s,(long)msgp,(long)cnt,(long)flags);}
    user_ssize_t sendmsg_x(int s,struct msghdr_x *msgp,u_int cnt,int flags) {return syscall(SENDMSG_X,(long)s,(long)msgp,(long)cnt,(long)flags);}
    ...
    user_ssize_t guarded_writev_np(int fd,const guardid_t *guard,struct iovec *iovp,int iovcnt) {return syscall(GUARDED_WRITEV_NP,(long)fd,(long)guard,(long)iovp,(long)iovcnt);}
    ...
    int persona(uint32_t operation,uint32_t flags,struct kpersona_info *info,uid_t *id,size_t *idlen,char *path) {return syscall(PERSONA,(long)operation,(long)flags,(long)info,(long)id,(long)idlen,(long)path);}
    ...
    int __nexus_open(struct nxctl_init *init,uint32_t init_len) {return syscall(__NEXUS_OPEN,(long)init,(long)init_len);}
    int __nexus_register(int ctl,struct nxprov_reg *reg,uint32_t reg_len,uuid_t *prov_uuid,uint32_t prov_uuid_len) {return syscall(__NEXUS_REGISTER,(long)ctl,(long)reg,(long)reg_len,(long)prov_uuid,(long)prov_uuid_len);}
    ...
    int __channel_open(struct ch_init *init,uint32_t init_len) {return syscall(__CHANNEL_OPEN,(long)init,(long)init_len);}
    int __channel_get_info(int c,struct ch_info *cinfo,uint32_t cinfolen) {return syscall(__CHANNEL_GET_INFO,(long)c,(long)cinfo,(long)cinfolen);}
    ...
    int setattrlistat(int fd,const char *path,struct attrlist *alist,void *attributeBuffer,size_t bufferSize,uint32_t options) {return syscall(SETATTRLISTAT,(long)fd,(long)path,(long)alist,(long)attributeBuffer,(long)bufferSize,(long)options);}
    int net_qos_guideline(struct net_qos_param *param,uint32_t param_len) {return syscall(NET_QOS_GUIDELINE,(long)param,(long)param_len);}
    int fmount(const char *type,int fd,int flags,void *data) {return syscall(FMOUNT,(long)type,(long)fd,(long)flags,(long)data);}
    int ntp_adjtime(struct timex *tp) {return syscall(NTP_ADJTIME,(long)tp);}
    int ntp_gettime(struct ntptimeval *ntvp) {return syscall(NTP_GETTIME,(long)ntvp);}
    ...
    int shared_region_map_and_slide_2_np(uint32_t files_count,const struct shared_file_np *files,uint32_t mappings_count,const shared_file_mapping_slide_np_ut *mappings_u) {return syscall(SHARED_REGION_MAP_AND_SLIDE_2_NP,(long)files_count,(long)files,(long)mappings_count,(long)mappings_u);}
    ...
    user_ssize_t sys_preadv(int fd,struct iovec *iovp,int iovcnt,off_t offset) {return syscall(SYS_PREADV,(long)fd,(long)iovp,(long)iovcnt,(long)offset);}
    user_ssize_t sys_pwritev(int fd,struct iovec *iovp,int iovcnt,off_t offset) {return syscall(SYS_PWRITEV,(long)fd,(long)iovp,(long)iovcnt,(long)offset);}
    user_ssize_t sys_preadv_nocancel(int fd,struct iovec *iovp,int iovcnt,off_t offset) {return syscall(SYS_PREADV_NOCANCEL,(long)fd,(long)iovp,(long)iovcnt,(long)offset);}
    user_ssize_t sys_pwritev_nocancel(int fd,struct iovec *iovp,int iovcnt,off_t offset) {return syscall(SYS_PWRITEV_NOCANCEL,(long)fd,(long)iovp,(long)iovcnt,(long)offset);}
