...
for PATH_SRC_CPP in $PATHS_CPP; do
    ...
    clang++ --target=x86_64-apple-darwin $COMPILE_VERSION $FLAGS_BOTH $FLAGS_COMP -I$PATH_INC -c "$PATH_SRC_CPP" -o "$PATH_OBJ_O"
done
...
for PATH_TST_CPP in $PATHS_CPP; do
    ...
    clang++ --target=x86_64-apple-darwin $COMPILE_VERSION $FLAGS_BOTH $FLAGS_COMP -I$PATH_INC -c "$PATH_TST_CPP" -o "$PATH_OBJ_O"
done
...
clang++ --target=x86_64-apple-darwin $COMPILE_VERSION  $FLAGS_BOTH $FLAGS_LINK $PATHS_O -o $PATH_EXE
